...
fn get_installation_directory() -> PathBuf {
    return PathBuf::from(data_local_dir().unwrap().to_str().unwrap()).join("Uriven");
}
...
async fn main() {
    ...
    let base_url : &str = "www.uriven.com";
    let mut setup_url : &str = "setup.uriven.com";
    let fallback_setup_url : &str = "setup.uriven.com";
    let mut bootstrapper_filename :&str = "UrivenPlayerLauncher.exe";
    #[cfg(not(target_os = "windows"))]
    {
        bootstrapper_filename = "UrivenPlayerLinuxLauncher";
    }
    let build_date = include_str!(concat!(env!("OUT_DIR"), "/build_date.txt"));
    let startup_text = format!(r###"
    888     888 8888888b.  8888888 888     888 8888888888 888b    888
    888     888 888   Y88b   888   888     888 888        8888b   888
    888     888 888    888   888   888     888 888        88888b  888
    888     888 888   d88P   888   Y88b   d88P 8888888    888Y88b 888
    888     888 8888888P""    888    Y88b d88P  888        888 Y88b888
    888     888 888 T88b     888     Y88o88P   888        888  Y88888
    Y88b. .d88P 888  T88b    888      Y888P    888        888   Y8888
     ""Y88888P""  888   T88b 8888888     Y8P     8888888888 888    Y888
   ...
   {} | Build Date: {} | Version: {}"###, base_url ,build_date, env!("CARGO_PKG_VERSION"));
    ...
    if terminal_width < 80 {
        print!("{}\n", format!("URIVEN Bootstrapper | {} | Build Date: {} | Version: {}", base_url, build_date, env!("CARGO_PKG_VERSION")).to_string().magenta().cyan().italic().on_black()); // Fallback message
    } else {
...
Name=Uriven Launcher
Exec={} %u
Icon={}
Type=Application
Terminal=true
Version={}
MimeType=x-scheme-handler/uriven-player;", latest_bootstrapper_path.to_str().unwrap(), latest_bootstrapper_path.to_str().unwrap(), env!("CARGO_PKG_VERSION"));
                
                let desktop_file_path = dirs::data_local_dir().unwrap().join("applications").join("uriven-player.desktop");
                std::fs::write(desktop_file_path, desktop_file_content).unwrap();

                info("Please launch URIVEN from the website, to continue with the update process.");
                std::thread::sleep(std::time::Duration::from_secs(20));
            }
            std::process::exit(0);
    ...
    if !app_settings_path.exists() { //|| !client_executable_path.exists() {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            let hkey_syntax_player = hkey_classes_root.create_subkey("uriven-player").unwrap().0;
            ...
            hkey_syntax_player.set_value("", &format!("URL: Uriven Protocol")).unwrap();
...
<Settings>
    ...
    if args.len() == 1 {
        ...
        #[cfg(target_os = "windows")]
        {
            std::process::Command::new("cmd").arg("/c").arg("start").arg("https://www.uriven.com/games").spawn().unwrap();
            std::process::exit(0);
        }
        #[cfg(not(target_os = "windows"))]
        {
            std::process::Command::new("xdg-open").arg("https://www.uriven.com/games").spawn().unwrap();
            std::process::exit(0);
        }
    }

    let main_args = &args[1];
    let main_args = main_args.replace("uriven-player://", "");
    ...
    if client_year == "2018" {
        client_executable_path = current_version_directory.join("Client2018").join("UrivenPlayerBeta.exe");
    } else if client_year == "2020" {
        client_executable_path = current_version_directory.join("Client2020").join("UrivenPlayerBeta.exe");
    } else if client_year == "2014" {
        client_executable_path = current_version_directory.join("Client2014").join("UrivenPlayerBeta.exe");
    } else if client_year == "2021" {
        client_executable_path = current_version_directory.join("Client2021").join("UrivenPlayerBeta.exe");
    } else {
        client_executable_path = current_version_directory.join("Client2016").join("UrivenPlayerBeta.exe");
    }
    if !client_executable_path.exists() {
        ...
        error("Failed to run UrivenPlayerBeta.exe, is your antivirus removing it? The bootstrapper will attempt to redownload the client on next launch.");
        std::thread::sleep(std::time::Duration::from_secs(20));
        std::process::exit(0);
    }
    let client_directory = client_executable_path.parent().unwrap();
    let app_settings_path = client_directory.join("AppSettings.xml");
    let content_tag = if client_year == "2018" { "ContentFold" } else { "ContentFolder" };
    if !app_settings_path.exists() {
        let app_settings_xml = format!(
            ...
            content_tag, content_tag, base_url
        );
    
        fs::write(&app_settings_path, app_settings_xml)
            .expect("Failed to create client-specific AppSettings.xml");
    }
    match launch_mode.as_str() {
        "play" => {
            info("Launching URIVEN");
            #[cfg(target_os = "windows")]
            {           
